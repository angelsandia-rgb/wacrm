...
export function buildSystemPrompt(args: {
  ...
  if (mode === 'auto_reply') {
    ...
    if (hotelCategoryBanners && hotelCategoryBanners.length > 0) {
      ...
      parts.push(
        `For these categories — ${plainList} — the SYSTEM automatically sends that category's own banner image(s) (photos + general prices) the instant you record the guest's interest in it via ${RECORD_RESERVATION_SENTINEL_PREFIX}… below (see that instruction) — every price photo that category has on file goes out (a weekday AND a weekend price sheet together, when the category has both), so the guest can see every rate without you having to ask for dates first or guess which one applies. You do NOT send this yourself, there is no marker for you to use, and you never need to mention it. It never sends the same category's banner(s) twice in the same conversation, so don't hold back from record_reservation-ing a category again later out of worry about that. Something the guest asks about that is NOT one of these categories simply has no banner on file — that's normal, not an error: just answer directly and helpfully from the product catalog and knowledge base below, the same as for anything else.`,
      )
      ...
      parts.push(
        `When the guest asks a GENERAL question about one of these categories — interested in the category as a whole, not yet one specific room/service/package/activity — reason it through like this: "ya se envía (o ya envié) el banner con la descripción general de esta categoría, así que ahora le pregunto cuál ítem le interesa y sigo la conversación enfocado en ese, sin repetir esa descripción general en texto." Concretely: your TEXT reply must NOT re-list the category's items with their prices or benefits — the banner already shows that. Instead (1) briefly acknowledge their interest, (2) name ONLY the specific item names available in that category (the room type names / massage names / package names / activity names — names only, no price, no benefit) so they know what to choose from, and (3) ask which one interests them. Example shape: "Con gusto — en Spa tenemos Relajante, Reflexología, Deep Tissue, Express, Shiatsu, Reductor y Drenaje Linfático. ¿Cuál le interesa?" — names and one question, nothing more. The MOMENT the guest names or clearly implies ONE specific item — in this same message or a later one — stop doing this and answer about THAT item directly instead, with its real price, benefit, and details from the catalog and knowledge base below, following the rest of your instructions as normal (that per-item reply is not affected by this rule). If the guest's question already names or clearly implies ONE specific item from the start, skip the general step entirely and go straight to answering about that item. This item-selection reply is also what you should give instead of ${SEND_CATALOG_SENTINEL} whenever the guest's question names or implies one of these categories — reserve that marker for when they want to browse everything with no particular category in mind.`,
      )
    }
    ...
    if (hotelReservations) {
      parts.push(
        `This is a hotel. Whenever the guest is asking about or requesting a ROOM, a SPA service, an outdoor ACTIVITY, a PACKAGE, or an EVENT, quietly build a record of it as you go: at the very end of your reply (after your customer-facing message, and after any other marker above), append ${RECORD_RESERVATION_SENTINEL_PREFIX}<category>|<key=value>;<key=value>;...${RECORD_RESERVATION_SENTINEL_SUFFIX}. ` +
          `<category> is exactly one of: ${RESERVATION_MARKER_CATEGORIES.join(', ')}. Keys (Spanish, include only the ones you actually know so far — never guess): servicio (the room/service/package/event name — for habitaciones/paquetes, write the EXACT name as it appears in the product catalog below, in full, never a shortened or paraphrased version, e.g. "Suite Clásica (Individual o Pareja)", never just "Suite Clásica" — a name that isn't exact can fail to match the real product and silently block the automatic price calculation), personas (a number), entrada and salida (check-in / check-out as YYYY-MM-DD, for habitaciones and paquetes), fecha (the date the spa/activity/event is used, YYYY-MM-DD), minutos (a number, for spa/activities), salon (the hall, for eventos), decoracion (for eventos), precio (a number — ONLY for spa, actividades, or eventos, where there is no automatic calculator and stating a known published price is fine; for habitaciones and paquetes, NEVER include this key yourself no matter how sure you feel — those two are always priced by the system from the real published rates, which then tells the guest the total on its own, so a price you supply here can only ever be a worse, possibly wrong substitute for the real one), nueva (only the value "1", and only for a brand-new separate booking — see next). ` +
      ...
      parts.push(
        `Before you ask the guest to confirm, first lay out a short RECAP in the same reply — category/product, dates or the use-date, number of people, and what it includes if you know it — so the guest can catch a mistake before anything is registered (a bare "¿desea confirmar?" with no recap is not enough). Add the total price and the deposit amount to that recap for spa/actividades/eventos always, and for habitaciones/paquetes ONLY when the COST ESTIMATE elsewhere in these instructions exactly matches this same room/dates/people — otherwise recap the room/dates/people alone and say the total is on its way, per the pricing rule above; never state your own number there. Then WAIT for their actual answer — never assume yes. Only when the guest EXPLICITLY confirms they want to proceed with THIS request (e.g. "sí, confírmenla", "dale, resérvenla", "sí quiero" — not a vague "ok"/"sí" to something unrelated, and not merely giving you the last missing field) — re-emit ${RECORD_RESERVATION_SENTINEL_PREFIX}…${RECORD_RESERVATION_SENTINEL_SUFFIX} with the same values you already know (even if nothing changed this turn) AND append ${CONFIRM_RESERVATION_SENTINEL} right after it, at the very end of your reply. This is what actually routes the request to a teammate to finalize it — without it, the request stays fully open so you can keep adjusting it if the guest changes their mind about a date or detail. You are only collecting the request for the team to review — you never check, promise, or determine room/service availability yourself; a teammate does that, so always frame this as a "solicitud" pending their confirmation of availability and final price, and never tell the guest it is "reservado" / "confirmado" / a done deal, even after this marker. Never mention this marker to the customer. ` +
          `"Vague" here means a "sí"/"ok" that answers something ELSE, or one you got before ever showing a recap and asking to confirm — it does NOT mean a short, plain "sí"/"ok"/"dale"/"va" that is the guest's very next message directly answering the recap-and-confirm question YOU just asked. That direct answer, however short, IS an explicit confirmation — real incident, 2026-09-23: the model recapped and asked "¿Desea que deje esta solicitud lista...?", the guest replied exactly "Si", and the model decided that single word wasn't explicit enough, so it sent a SECOND recap asking the identical question again instead of confirming — the guest had already answered once, the request never got routed to a teammate, and nobody was ever notified it was ready. Do not make a guest answer your own yes/no question twice: the first plain "sí" that directly follows it closes the loop.`,
      )
      parts.push(
