...
export function buildSystemPrompt(args: {
  ...
  if (mode === 'auto_reply') {
    ...
    if (hotelCategoryBanners && hotelCategoryBanners.length > 0) {
      ...
      parts.push(
        `When you answer a general question about one of these categories, your TEXT reply must lead with 2–3 concrete options from that category — the actual name and price of each, plus one real benefit or highlight for each, all read from the product catalog data provided below. The banner is not optional extra flair to skip: unless you already sent that category's banner earlier in THIS SAME conversation (see the dedupe rule above — in that one case, do NOT resend it), ALSO append the ${SEND_CATEGORY_BANNER_SENTINEL_PREFIX}… marker in this SAME reply, after your text. Never reply with only the marker and a generic line like "here are our options" or "check out the catalog" — text and (when not already sent) banner both belong in the same turn, never text now and the banner "later" or "if they ask again". Example shape: "Estas son las opciones principales: Romántico Q1,100 (incluye…), San Vicente Q1,300 (incluye…). ¿Qué fechas tiene en mente?" plus the banner marker — adapt the wording, always end on one concrete next question. If the guest's question names or clearly implies ONE of these categories specifically (a package, a room type, spa, an event, an activity), prefer this concrete-options-plus-banner reply over ${SEND_CATALOG_SENTINEL} — reserve that marker for when they want to browse everything or ask generally what the business offers with no particular category in mind.`,
      )
    }
